...
class Statistics extends Descriptor {
  ...
  def columnStats(columnName: String, columnStats: ColumnStats): Statistics = {
    val map = mutable.Map(NormalizedProperties.normalizeColumnStats(columnStats).toSeq: _*)
  ...
  def columnDistinctCount(columnName: String, ndv: Long): Statistics = {
    this.columnStats
      ...
      .put("distinct-count", ndv.toString)
    this
  }
  ...
  def columnNullCount(columnName: String, nullCount: Long): Statistics = {
    this.columnStats
      .getOrElseUpdate(columnName, mutable.HashMap())
      .put("null-count", nullCount.toString)
    this
  }
  ...
  def columnAvgLength(columnName: String, avgLen: Double): Statistics = {
    this.columnStats
      .getOrElseUpdate(columnName, mutable.HashMap())
      .put("avg-length", avgLen.toString)
    this
  }
  ...
  def columnMaxLength(columnName: String, maxLen: Integer): Statistics = {
    this.columnStats
      .getOrElseUpdate(columnName, mutable.HashMap())
      .put("max-length", maxLen.toString)
    this
  }
  ...
  final override def addProperties(properties: NormalizedProperties): Unit = {
    rowCount.foreach(rc => properties.putLong(statistics("row-count"), rc))
