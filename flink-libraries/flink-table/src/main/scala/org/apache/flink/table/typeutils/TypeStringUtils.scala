...
object TypeStringUtils extends JavaTokenParsers with PackratParsers {
  ...
  lazy val fieldName: PackratParser[String] = stringLiteral | ident
  ...
  lazy val any: PackratParser[TypeInformation[_]] =
    ANY ~ "(" ~ qualifiedName ~ "," ~ base64 ~ ")" ^^ {
      case _ ~ _ ~ typeInfoClass ~ _ ~ serialized ~ _=>
        val clazz = loadClass(typeInfoClass)
        val typeInfo = deserialize(serialized)

        if (!clazz.isInstance(typeInfo)) {
          throw new ValidationException(
            s"Class '$typeInfoClass' does no correspond to serialized data.")
        }
        typeInfo
    }

  lazy val typeInfo: PackratParser[TypeInformation[_]] =
    namedRow | unnamedRow | generic | atomic | failure("Invalid type.")
  ...
  def writeTypeInfo(typeInfo: TypeInformation[_]): String = typeInfo match {
    ...
    case rt: RowTypeInfo =>
      ...
      val normalizedFields = fields.map { f =>
        s"${StringEscapeUtils.escapeCsv(f._1)} ${normalizeTypeInfo(f._2)}"
      }
      s"${ROW.key}(${normalizedFields.mkString(", ")})"

    case generic: GenericTypeInfo[_] =>
      s"${ANY.key}(${generic.getTypeClass.getCanonicalName})"
    ...
    case any: TypeInformation[_] =>
      s"${ANY.key}(${any.getClass.getCanonicalName}, ${serialize(any)})"
  }
    ...
    try {
      ...
      case e: Exception =>
        throw new ValidationException(s"Unable to serialize type information '$obj' with " +
          s"class '${obj.getClass.getCanonicalName}'.", e)
    }
  }
