...
case class Project(projectList: Seq[NamedExpression], child: LogicalNode) extends UnaryNode {
  ...
  override def validate(tableEnv: TableEnvironment): LogicalNode = {
    ...
    def checkName(name: String): Unit = {
      ...
      } else if (tableEnv.isInstanceOf[StreamTableEnvironment]
          && (name == "rowtime" || name == "systemtime")) {
        failValidation("'rowtime' and 'systemtime' cannot be used as field name in a " +
  ...
  override def resolveExpressions(tableEnv: TableEnvironment): LogicalNode = {
    if (aliasList.length > child.output.length) {
      ...
      failValidation("Alias can not accept '*' as name")
          ...
          case UnresolvedFieldReference(name) => name != "rowtime" && name != "systemtime"
        }) {
      failValidation("'rowtime' and 'systemtime' cannot be used as field name " +
...
case class WindowAggregate(
  ...
  override def resolveReference(
    ...
    case _: StreamTableEnvironment if name == "systemtime" =>
      Some(SystemtimeAttribute())
    case _ =>
      window.alias match {
