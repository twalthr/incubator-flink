    ...
    val preparedDataSet = if (isTimeWindow) {
      ...
      if (groupingKeys.length > 0) {
        ...
        if (isIncremental) {
          ...
          throw new UnsupportedOperationException(
            "Count sliding grouping windows on event-time with non-incremental aggregates " +
              "are currently not supported.")
        }
      }
      // all window
      else {
        // TODO: count sliding all window on event-time should sort all the data set
        // on event time before applying the windowing logic.
        throw new UnsupportedOperationException(
          "Count sliding non-grouping windows on event-time are currently not supported.")
      }
    }
