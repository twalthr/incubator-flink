...
trait DataStreamRel extends RelNode with FlinkRelNode {
  ...
  /**
    * Returns array of indices mapping to logical row fields.
    */
  def getPhysicalRowTypeMapping: Array[Int] = {
    getPhysicalFieldNames.map(name => getRowType.getFieldNames.indexOf(name)).toArray
  }
