...
case class SystemtimeAttribute() extends Attribute {
  override private[flink] def withName(newName: String): Attribute = {
    if (newName == "systemtime") {
      this
    } else {
      throw new ValidationException("Cannot rename streaming systemtime attribute.")
    }
  }

  override private[flink] def name: String = "systemtime"
