...
import org.apache.flink.table.descriptors.DescriptorUtils.metadata
...
class Metadata extends Descriptor {
  ...
  final override def addProperties(properties: NormalizedProperties): Unit = {
    comment.foreach(c => properties.putString(metadata("comment"), c))
    creationTime.foreach(t => properties.putLong(metadata("creation-time"), t))
    lastAccessTime.foreach(t => properties.putLong(metadata("last-access-time"), t))
  }
}
