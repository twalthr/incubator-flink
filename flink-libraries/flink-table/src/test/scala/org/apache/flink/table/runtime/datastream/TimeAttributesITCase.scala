...
class TimeAttributesITCase extends StreamingMultipleProgramsTestBase {
  ...
  def testPojoSupport(): Unit = {
    ...
    val table = stream.toTable(tEnv, ('b as 'b).rowtime, 'c as 'c, 'a as 'a, 'xxx.proctime)
    // no aliases, no swapping
    val table2 = stream.toTable(tEnv, 'a, 'b.rowtime, 'c, 'xxx.proctime)

    val t = table.select('b, 'c , 'a).unionAll(table2.select('b, 'c, 'a))
