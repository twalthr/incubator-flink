...
public class CliChangelogResultView extends CliResultView<CliChangelogResultView.ResultChangelogOperation> {
	...
	protected void refresh() {
		...
		final TypedResult<Tuple2<Boolean, Row>> result;
		try {
			result = client.getExecutor().retrieveResultRecord(client.getContext(), resultDescriptor.getResultId());
		...
		switch (result.getType()) {
			...
			case EOS:
				...
				break;
			default:
				Tuple2<Boolean, Row> change = result.getPayload();
