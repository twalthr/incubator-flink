...
public class LocalExecutor implements Executor {
	...
	public ResultDescriptor executeQuery(SessionContext context, String query) throws SqlExecutionException {
		...
		final Tuple2<JobGraph, TableSchema> plan = createPlan(context.getName(), env, query, result.getTableSink(), clusterClient);

		// store the result with a unique id (the job id for now)
		final String resultId = plan.f0.getJobID().toString();
		...
		final Runnable program = () -> {
			...
			try {
				clusterClient.run(plan.f0, classLoader);
		...
		return new ResultDescriptor(resultId, plan.f1, result.isChangelog());
	}

	@Override
	public TypedResult<Tuple2<Boolean, Row>> retrieveResultRecord(SessionContext context, String resultId) throws SqlExecutionException {
	...
	public List<Row> retrieveResultPage(String resultId, int page) throws SqlExecutionException {
		...
		return result.retrievePage(page);
	}
	...
	private Tuple2<JobGraph, TableSchema> createPlan(String name, Environment env, String query, TableSink<?> sink, ClusterClient<?> clusterClient) {
		final TableEnvironment tableEnv = getTableEnvironment(env);
		...
		if (env.getExecution().isStreamingExecution()) {
			...
			unoptimizedPlan.setJobName(name + ": " + query);
		...
		// create job graph
		final JobGraph jobGraph = clusterClient.getJobGraph(
			plan, dependencies, Collections.emptyList(), SavepointRestoreSettings.none());

		return Tuple2.of(jobGraph, table.getSchema());
	}
