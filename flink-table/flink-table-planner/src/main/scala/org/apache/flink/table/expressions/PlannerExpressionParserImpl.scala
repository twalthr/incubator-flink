...
object PlannerExpressionParserImpl extends JavaTokenParsers
  ...
  lazy val currentRange: PackratParser[Expression] = CURRENT_RANGE ^^ {
    _ => untypedCall(BuiltInFunctionDefinitions.CURRENT_RANGE)
  }

  lazy val currentRow: PackratParser[Expression] = CURRENT_ROW ^^ {
    _ => untypedCall(BuiltInFunctionDefinitions.CURRENT_ROW)
  }

  lazy val unboundedRange: PackratParser[Expression] = UNBOUNDED_RANGE ^^ {
    _ => untypedCall(BuiltInFunctionDefinitions.UNBOUNDED_RANGE)
  }

  lazy val unboundedRow: PackratParser[Expression] = UNBOUNDED_ROW ^^ {
    _ => untypedCall(BuiltInFunctionDefinitions.UNBOUNDED_ROW)
  }
  ...
  lazy val over: PackratParser[Expression] = composite ~ OVER ~ fieldReference ^^ {
    case agg ~ _ ~ windowRef =>
      untypedCall(BuiltInFunctionDefinitions.OVER, agg, windowRef)
  }

  // suffix operators

  lazy val suffixAsc : PackratParser[Expression] = composite <~ "." ~ ASC ~ opt("()") ^^ { e =>
      untypedCall(BuiltInFunctionDefinitions.ORDER_ASC, e)
  }

  lazy val suffixDesc : PackratParser[Expression] = composite <~ "." ~ DESC ~ opt("()") ^^ { e =>
    untypedCall(BuiltInFunctionDefinitions.ORDER_DESC, e)
  }

  lazy val suffixCast: PackratParser[Expression] =
    composite ~ "." ~ CAST ~ "(" ~ dataType ~ ")" ^^ {
      case e ~ _ ~ _ ~ _ ~ dt ~ _ =>
        untypedCall(BuiltInFunctionDefinitions.CAST, e, typeLiteral(fromLegacyInfoToDataType(dt)))
    }
  ...
  lazy val suffixLog: PackratParser[Expression] =
    composite ~ "." ~ LOG ~ "(" ~ expression ~ ")" ^^ {
      case operand ~ _ ~ _ ~ _ ~ base ~ _ =>
        untypedCall(BuiltInFunctionDefinitions.LOG, base, operand)
    }
  ...
  lazy val prefixTimestampDiff: PackratParser[Expression] =
    TIMESTAMP_DIFF ~ "(" ~ timePointUnit ~ "," ~ expression ~ "," ~ expression ~ ")" ^^ {
      case _ ~ _ ~ unit ~ _ ~ operand1 ~ _ ~ operand2 ~ _ =>
        untypedCall(BuiltInFunctionDefinitions.TIMESTAMP_DIFF, unit, operand1, operand2)
    }
  ...
  lazy val prefixDistinct: PackratParser[Expression] =
    functionIdent ~ "." ~ DISTINCT ~ "(" ~ repsep(expression, ",") ~ ")" ^^ {
      case name ~ _ ~ _ ~ _ ~ args ~ _ =>
        untypedCall(BuiltInFunctionDefinitions.DISTINCT, lookupCall(name, args: _*))
    }
  ...
  lazy val unaryNot: PackratParser[Expression] = "!" ~> composite ^^ { e =>
    untypedCall(BuiltInFunctionDefinitions.NOT, e)
  }

  lazy val unaryMinus: PackratParser[Expression] = "-" ~> composite ^^ { e =>
    untypedCall(BuiltInFunctionDefinitions.MINUS_PREFIX, e)
  }
  ...
  lazy val product: PackratParser[Expression] = unary * (
    "*" ^^^ { (a:Expression, b:Expression) => untypedCall(BuiltInFunctionDefinitions.TIMES, a, b) } |
    "/" ^^^ { (a:Expression, b:Expression) => untypedCall(BuiltInFunctionDefinitions.DIVIDE, a, b) } |
    "%" ^^^ { (a:Expression, b:Expression) => untypedCall(BuiltInFunctionDefinitions.MOD, a, b) } ) |
    failure("Product expected.")

  lazy val term: PackratParser[Expression] = product * (
    "+" ^^^ { (a:Expression, b:Expression) => untypedCall(BuiltInFunctionDefinitions.PLUS, a, b) } |
    "-" ^^^ { (a:Expression, b:Expression) => untypedCall(BuiltInFunctionDefinitions.MINUS, a, b) } ) |
    failure("Term expected.")

  // comparison

  lazy val equalTo: PackratParser[Expression] = term ~ ("===" | "==" | "=") ~ term ^^ {
    case l ~ _ ~ r => untypedCall(BuiltInFunctionDefinitions.EQUALS, l, r)
  }

  lazy val notEqualTo: PackratParser[Expression] = term ~ ("!==" | "!=" | "<>") ~ term ^^ {
    case l ~ _ ~ r => untypedCall(BuiltInFunctionDefinitions.NOT_EQUALS, l, r)
  }

  lazy val greaterThan: PackratParser[Expression] = term ~ ">" ~ term ^^ {
    case l ~ _ ~ r => untypedCall(BuiltInFunctionDefinitions.GREATER_THAN, l, r)
  }

  lazy val greaterThanOrEqual: PackratParser[Expression] = term ~ ">=" ~ term ^^ {
    case l ~ _ ~ r => untypedCall(BuiltInFunctionDefinitions.GREATER_THAN_OR_EQUAL, l, r)
  }

  lazy val lessThan: PackratParser[Expression] = term ~ "<" ~ term ^^ {
    case l ~ _ ~ r => untypedCall(BuiltInFunctionDefinitions.LESS_THAN, l, r)
  }

  lazy val lessThanOrEqual: PackratParser[Expression] = term ~ "<=" ~ term ^^ {
    case l ~ _ ~ r => untypedCall(BuiltInFunctionDefinitions.LESS_THAN_OR_EQUAL, l, r)
  }
  ...
  lazy val logic: PackratParser[Expression] = comparison * (
    "&&" ^^^ { (a:Expression, b:Expression) => untypedCall(BuiltInFunctionDefinitions.AND, a, b) } |
    "||" ^^^ { (a:Expression, b:Expression) => untypedCall(BuiltInFunctionDefinitions.OR, a, b) } ) |
  ...
  lazy val proctime: PackratParser[Expression] = fieldReference ~ "." ~ PROCTIME ^^ {
    case f ~ _ ~ _ => untypedCall(BuiltInFunctionDefinitions.PROCTIME, f)
  }

  lazy val rowtime: PackratParser[Expression] = fieldReference ~ "." ~ ROWTIME ^^ {
    case f ~ _ ~ _ => untypedCall(BuiltInFunctionDefinitions.ROWTIME, f)
  }
  ...
  lazy val alias: PackratParser[Expression] = logic ~ AS ~ fieldReference ^^ {
      case e ~ _ ~ name =>
        untypedCall(BuiltInFunctionDefinitions.AS, e, valueLiteral(name.getName))
  ...
  lazy val aliasMapping: PackratParser[Expression] =
    fieldReference ~ AS ~ fieldReference ^^ {
      case e ~ _ ~ name => untypedCall(BuiltInFunctionDefinitions.AS, e, valueLiteral(name.getName))
  }

  // columns

  lazy val fieldNameRange: PackratParser[Expression] = fieldReference ~ TO ~ fieldReference ^^ {
    case start ~ _ ~ end => untypedCall(BuiltInFunctionDefinitions.RANGE_TO, start, end)
  }
