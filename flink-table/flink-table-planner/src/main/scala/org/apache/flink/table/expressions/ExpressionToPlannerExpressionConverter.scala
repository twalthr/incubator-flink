...
/**
  ...
  */
class ExpressionToPlannerExpressionConverter private extends ExpressionVisitor[Expression] {
    ...
    func match {
      ...
      case fd: FunctionDefinition =>
        fd match {
          ...
          case IN =>
            assert(args.size > 1)
            In(args.head, args.slice(1, args.size))
  ...
  override def visit(other: CommonExpression): Expression = {
    other match {
      case e: TableReferenceExpression =>
        TableReference(
...
object ExpressionToPlannerExpressionConverter {
  val INSTANCE: ExpressionToPlannerExpressionConverter = new ExpressionToPlannerExpressionConverter
}
