...
object ExpressionUtils {
  ...
  private[flink] def typeLiteral(t: TypeInformation[_]): TypeLiteralExpression = {
    new TypeLiteralExpression(t)
  }

  private[flink] def symbol(symbol: TableSymbol): SymbolExpression = {
    new SymbolExpression(symbol)
  }

  private[flink] def fieldRef(name: String): FieldReferenceExpression = {
    new FieldReferenceExpression(name)
  }

  private[flink] def tableRef(name: String, table: Table): TableReferenceExpression = {
