 ...
 *
 * @deprecated This class has been replaced by {@link Schema} (which represents a builder-like API
 *     class) and {@link ResolvedSchema} (which represents the fully resolved schema). This class
 *     can be dropped once we remove legacy interfaces (such as {@link TableSource}) that still
 *     reference it.
 */
@Deprecated
@PublicEvolving
public class TableSchema extends ResolvedSchema {
    ...
    private TableSchema(
        ...
        super(columns, watermarkSpecs, primaryKey);
    }

    private static List<TableColumn> createTableColumns(
            String[] fieldNames, TypeInformation<?>[] fieldTypes) {
        final DataType[] fieldDataTypes = fromLegacyInfoToDataType(fieldTypes);
    ...
    public TableSchema(String[] fieldNames, TypeInformation<?>[] fieldTypes) {
        super(createTableColumns(fieldNames, fieldTypes), Collections.emptyList(), null);
        validateColumnsAndWatermarkSpecs(columns, watermarkSpecs);
    }
