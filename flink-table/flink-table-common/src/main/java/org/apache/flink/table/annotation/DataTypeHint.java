 ...
 *
 * The following examples show how to explicitly specify data types, how to parameterize the extraction
 * logic, or how to accept any data type as an input data type:
 *
 * <p>{@code @DataTypeHint("INT")} defines an INT data type with a reflectively extracted conversion class.
 *
 ...
 * {@link java.math.BigDecimal} will be extracted as {@code DECIMAL(12, 2)} if the enclosing class is
 * annotated with {@code @DataTypeHint(defaultDecimalPrecision = 12, defaultDecimalScale = 2)}. Individual
...
public @interface DataTypeHint {
	...
	/**
	 * The string representation of a data type. See {@link DataTypes} for a list of supported data
	 * types. For example, {@code INT} for an integer data type or {@code DECIMAL(12, 5)} for decimal
	 ...
	 * the opaque RAW type. It is only used if {@link #value()} is defined as an unparameterized {@code RAW}
	 * string.
	 *
