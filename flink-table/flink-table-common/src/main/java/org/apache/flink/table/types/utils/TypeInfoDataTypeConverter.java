...
public class TypeInfoDataTypeConverter {
	...
	public static DataType toDataType(TypeInformation<?> typeInfo) {
		return convertToDataType(typeInfo);
	}

	public static DataType[] toDataType(TypeInformation<?>[] typeInfo) {
		return Stream.of(typeInfo)
			.map(TypeInfoDataTypeConverter::toDataType)
	...
	private static DataType convertToDataType(TypeInformation<?> typeInfo) {
		return convertToLogicalType(typeInfo).bridgedTo(typeInfo.getTypeClass());
	}

	private static DataType convertToLogicalType(TypeInformation<?> typeInfo) {
		...
		if (typeInfo.equals(Types.STRING)) {
			...
			final DataType elementType =
				convertToDataType(((PrimitiveArrayTypeInfo<?>) typeInfo).getComponentType());
			return DataTypes.ARRAY(elementType).notNull();
		}

		else if (typeInfo instanceof BasicArrayTypeInfo) {
			final DataType elementType =
				convertToDataType(((BasicArrayTypeInfo<?, ?>) typeInfo).getComponentInfo()).nullable();
			return DataTypes.ARRAY(elementType).notNull();
		}

		else if (typeInfo instanceof ObjectArrayTypeInfo) {
			final DataType elementType =
				convertToDataType(((ObjectArrayTypeInfo<?, ?>) typeInfo).getComponentInfo()).nullable();
			return DataTypes.ARRAY(elementType).notNull();
		}

		else if (typeInfo instanceof MultisetTypeInfo) {
			// null if the element supports it
			final DataType elementType = convertToDataType(((MultisetTypeInfo<?>) typeInfo).getElementTypeInfo());
			...
			final DataType keyType =
				convertToDataType(((MapTypeInfo<?, ?>) typeInfo).getKeyTypeInfo());
			final DataType valueType =
				convertToDataType(((MapTypeInfo<?, ?>) typeInfo).getValueTypeInfo()).nullable();
			return DataTypes.MAP(keyType, valueType).notNull();
		}

		else if (typeInfo instanceof RowTypeInfo) {
			return convertToRowType((CompositeType<?>) typeInfo);
		}
		...
		// types for back-and-forth conversion
			...
			return convertToStructuredType((CompositeType) typeInfo);
		}
	...
	private static DataType convertToRowType(CompositeType<?> compositeType) {
		return DataTypes.ROW(extractFields(compositeType).toArray(new DataTypes.Field[0]))
			.notNull();
	}

	private static DataType convertToStructuredType(CompositeType<?> compositeType) {
		...
		final List<DataTypes.Field> fields = extractFields(compositeType);
	...
	private static List<DataTypes.Field> extractFields(CompositeType<?> compositeType) {
		...
		return IntStream.range(0, compositeType.getArity())
			.mapToObj(i -> {
				...
				DataType fieldType = convertToDataType(compositeType.getTypeAt(i));
