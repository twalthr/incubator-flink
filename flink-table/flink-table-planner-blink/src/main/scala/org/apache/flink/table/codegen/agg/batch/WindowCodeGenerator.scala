...
object WindowCodeGenerator {
  ...
  def asLong(expr: Expression): Long = expr match {
    case literal: ValueLiteralExpression
      if fromDataTypeToLegacyInfo(literal.getDataType) == TimeIntervalTypeInfo.INTERVAL_MILLIS ||
          fromDataTypeToLegacyInfo(literal.getDataType) == Types.LONG =>
  ...
  def isTimeIntervalLiteral(expr: Expression): Boolean = expr match {
    case literal: ValueLiteralExpression
      if fromDataTypeToLegacyInfo(literal.getDataType) == TimeIntervalTypeInfo.INTERVAL_MILLIS => true
    case _ => false
  }
}
