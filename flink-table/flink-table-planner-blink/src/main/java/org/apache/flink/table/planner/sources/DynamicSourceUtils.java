...
public final class DynamicSourceUtils {
				...
				throw new ValidationException(
					String.format(
						"Invalid data type for metadata key '%s' in column '%s' of table '%s'. " +
							"The produced metadata type '%s' cannot be cast to declared type '%s'.",
						metadataKey,
						c.getName(),
						sourceIdentifier.asSummaryString(),
	...
	private static void validateScanSource(
		...
		validateWatermarks(sourceIdentifier, schema, changelogMode);
	...
	private static void validateWatermarks(
		...
		if (!changelogMode.containsOnly(RowKind.INSERT)) {
			throw new TableException(
				String.format(
					"Defining WATERMARK for table '%s' on a table source that emits " +
						"more than INSERT-only changes is currently not supported.",
					sourceIdentifier.asSummaryString()
				)
			);
		}
