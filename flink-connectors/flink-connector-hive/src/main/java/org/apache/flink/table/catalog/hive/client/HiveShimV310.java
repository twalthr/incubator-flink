...
public class HiveShimV310 extends HiveShimV235 {
            ...
            synchronized (HiveShimV310.class) {
                if (!hiveClassesInited) {
                    try {
                        ...
                        hiveTimestampConstructor =
                                hiveTimestampClz.getDeclaredConstructor(LocalDateTime.class);
                        hiveTimestampConstructor.setAccessible(true);
                        hiveTimestampLocalDateTime =
                                hiveTimestampClz.getDeclaredField("localDateTime");
                        hiveTimestampLocalDateTime.setAccessible(true);
                        timestampWritableConstructor =
                                Class.forName(
                        ...
                        dateWritableConstructor =
                                Class.forName("org.apache.hadoop.hive.serde2.io.DateWritableV2")
            ...
            List<?> constraints =
                    (List<?>)
                            HiveReflectionUtils.invokeMethod(
                                    ...
                                    new Object[] {request});
            Class constraintClz =
                    Class.forName("org.apache.hadoop.hive.metastore.api.SQLNotNullConstraint");
    ...
    public LocalDateTime toFlinkTimestamp(Object hiveTimestamp) {
        ...
        Preconditions.checkArgument(
                hiveTimestampClz.isAssignableFrom(hiveTimestamp.getClass()),
                "Expecting Hive timestamp to be an instance of %s, but actually got %s",
                hiveTimestampClz.getName(),
                hiveTimestamp.getClass().getName());
        try {
    ...
    public LocalDate toFlinkDate(Object hiveDate) {
        initDateTimeClasses();
        Preconditions.checkArgument(
                hiveDateClz.isAssignableFrom(hiveDate.getClass()),
    ...
    List<Object> createHiveNNs(
        ...
        if (!nnCols.isEmpty()) {
            Preconditions.checkArgument(
                    nnCols.size() == traits.size(), "Number of NN columns and traits mismatch");
            Class nnClz =
                    Class.forName("org.apache.hadoop.hive.metastore.api.SQLNotNullConstraint");
            // NN constructor takes catName, dbName, tableName, colName, nnName, enable, validate,
            // rely
            Constructor constructor =
                    nnClz.getConstructor(
                            String.class,
                            String.class,
                            String.class,
                            String.class,
    ...
    String getHMSDefaultCatalog(Configuration conf)
        ...
        // HMS catalog (https://issues.apache.org/jira/browse/HIVE-18685) is an on-going feature and
        // we currently
