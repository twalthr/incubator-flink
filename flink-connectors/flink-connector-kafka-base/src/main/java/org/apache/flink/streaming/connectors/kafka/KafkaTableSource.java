...
public abstract class KafkaTableSource
	...
	public void withBoundedOutOfOrderRowtimeAttribute(String rowtime, Time delay) {
		...
		this.timestampAssigner = new RowFieldWatermarkAssigner(this.rowtimeFieldPos, delay.toMilliseconds());
	}
	...
	private String fieldInfo(String[] fieldNames, TypeInformation[] fieldTypes) {
		...
		for (int i = 0; i < fieldNames.length - 1; i++) {
			...
			sb.append(": ");
			if (fieldTypes[i] == Types.BOOLEAN) { sb.append("BOOLEAN"); }
			else if (fieldTypes[i] == Types.BYTE) { sb.append("TINYINT"); }
			else if (fieldTypes[i] == Types.SHORT) { sb.append("SMALLINT"); }
			else if (fieldTypes[i] == Types.INT) { sb.append("INTEGER"); }
			else if (fieldTypes[i] == Types.LONG) { sb.append("BIGINT"); }
			else if (fieldTypes[i] == Types.FLOAT) { sb.append("FLOAT"); }
			else if (fieldTypes[i] == Types.DOUBLE) { sb.append("DOUBLE"); }
			else if (fieldTypes[i] == Types.STRING) { sb.append("VARCHAR"); }
			else if (fieldTypes[i] == Types.DECIMAL) { sb.append("DECIMAL"); }
			else if (fieldTypes[i] == Types.SQL_DATE) { sb.append("DATE"); }
			else if (fieldTypes[i] == Types.SQL_TIME) { sb.append("TIME"); }
			else if (fieldTypes[i] == Types.SQL_TIMESTAMP) { sb.append("TIMESTAMP"); }
			else { sb.append(fieldTypes[i].getTypeClass().getSimpleName()); }
			sb.append(", ");
		}
